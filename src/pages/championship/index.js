...
// Ícone do lance (bola/cartão) pra exibir na súmula, e o rótulo em texto puro pro log de
// auditoria (que é só texto — SVG não faz sentido lá).
function eventIconHTML(type) {
  return type === 'goal' ? icon('ball', 16) : cardChip(type === 'yellow' ? 'var(--warning)' : 'var(--danger)');
}
function eventLabel(type) {
  return type === 'goal' ? 'Gol' : type === 'yellow' ? 'Cartão amarelo' : 'Cartão vermelho';
}
...
function sumulaModal(kind, id, store, { persist, addAudit }) {
  ...
  const box = document.getElementById('modalBox');
  box.querySelector('[data-close-modal]').onclick = () => { closeModal(); };
  box.querySelectorAll('[data-sumula-add]').forEach((button) => button.onclick = async () => { const [teamId, athleteId, type] = button.dataset.sumulaAdd.split(':'); store.addMatchEvent(obj, { type, teamId, athleteId }); await persist(); await addAudit(store.getState().id, 'match_event_added', `${eventLabel(type)}: ${athleteId} (${teamById(state, teamId)?.nome || '—'})`); sumulaModal(kind, id, store, { persist, addAudit }); });
  box.querySelectorAll('[data-sumula-anon]').forEach((button) => button.onclick = async () => { const [teamId, type] = button.dataset.sumulaAnon.split(':'); store.addMatchEvent(obj, { type, teamId, name: '' }); await persist(); await addAudit(store.getState().id, 'match_event_added', `${eventLabel(type)} s/ atleta (${teamById(state, teamId)?.nome || '—'})`); sumulaModal(kind, id, store, { persist, addAudit }); });
