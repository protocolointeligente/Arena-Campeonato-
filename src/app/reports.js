...
export function viewRelatoriosHTML(state) {
  ...
  return `<div class="card pad"><h2 style="font-size:25px;display:flex;align-items:center;gap:8px">${icon('fileText', 22)} Central de documentos</h2><p class="muted">Gere documentos operacionais separados para a categoria e fase atualmente selecionadas.</p><div class="banner"><b>${esc(activeCategory(state)?.nome || '')}</b><span class="muted">›</span>${esc(activePhaseOf(activeCategory(state))?.nome || '')}<span style="margin-left:auto" class="tag">PDF</span></div></div><div class="report-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-top:14px">${card(icon('shield', 26), 'Equipes', 'Relação de equipes, quantidade de atletas e técnico.', 'teams')}${card(icon('users', 26), 'Atletas', 'Relação nominal de atletas por equipe.', 'roster')}${card(icon('calendar', 26), 'Tabela oficial', 'Agenda com datas, horários, locais e arbitragem.', 'schedule')}${card(icon('trophy', 26), 'Classificação', 'Classificação oficial com critérios da competição.', 'standings')}${card(icon('ball', 26), 'Artilharia', 'Ranking de goleadores da competição.', 'scorers')}${card(icon('shieldCheck', 26), 'Disciplina', 'Cartões, pontuação disciplinar negativa e suspensões.', 'discipline')}${card(icon('whistle', 26), 'Arbitragem', 'Escala de árbitros, mesários e locais.', 'officials')}${card(icon('checkCircle', 26), 'Resultados', 'Resultados oficiais das partidas encerradas.', 'results')}<div class="card pad"><div style="font-size:26px">${icon('fileText', 26)}</div><h3 style="font-size:18px;margin-top:7px">Boletim da rodada</h3><p class="muted">Resultados da rodada e resumo da artilharia.</p><select data-report-round>${rounds.map((r) => `<option value="${r}">${r}ª rodada</option>`).join('') || '<option value="1">1ª rodada</option>'}</select><button class="btn ghost sm" data-export="round">Gerar PDF</button></div><div class="card pad"><div style="font-size:26px">${icon('clipboard', 26)}</div><h3 style="font-size:18px;margin-top:7px">Súmula completa</h3><p class="muted">PDF completo com classificação, jogos, chaveamento, artilharia e disciplina.</p><button class="btn ghost sm" data-export="pdf">Gerar PDF</button></div><div class="card pad"><div style="font-size:26px">${icon('creditCard', 26)}</div><h3 style="font-size:18px;margin-top:7px">Carteirinhas de atletas</h3><p class="muted">Gera credenciais individuais para cada atleta.</p><button class="btn ghost sm" data-export="cards">Gerar PDF</button></div>`;
}
